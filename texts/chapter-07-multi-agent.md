...
<figure>
<img src="../imgs/fig.7.1.Example_of_multi-agent_system.png" alt="Fig.1: Example of multi-agent system">
<figcaption>
...
<figure>
<img src="../imgs/fig.7.2.Agents_communicate_and_interact_in_various_ways.png" alt="Fig. 2: Agents communicate and interact in various ways.">
<figcaption>
...
def setup_environment() -> None:
    """Load environment variables and ensure GOOGLE_API_KEY is present."""
    load_dotenv()
...
def main() -> None:
    """
    Initialize and run the AI crew for content creation using a Gemini model.
    """
    setup_environment()

    # Define the language model to use.
    # Updated to a model from the Gemini 2.0 series for better
    # performance and features. For preview features consider:
    #   "gemini-2.5-flash"
    ...
    blog_creation_crew = Crew(
        ...
        llm=llm,
        verbose=2,  # Detailed crew execution logs
    )
...
process_step = LlmAgent(
    ...
    instruction=(
        ...
        "If you are the final step, update session state by "
...
# repeatedly until the status is 'completed'
...
# Step 1: This agent's output will be saved to session.state["data"]
step1 = Agent(
    name="Step1_Fetch",
    output_key="data",
)

# Step 2: This agent will use the data from the previous step.
...
# 1. Step1 will execute, and its response will be stored in session.state["data"].
# 2. Step2 will execute next, using the information from the state as instructed.
...
# For simplicity in this example, we'll embed the logic directly
# in the agent's instruction. In a real-world scenario, you would use tools.

weather_fetcher = Agent(
    name="weather_fetcher",
    model="gemini-2.0-flash-exp",
    instruction=(
        "Fetch the weather for the given location and return only "
        "the weather report."
    ),
...
news_fetcher = Agent(
    ...
    model="gemini-2.0-flash-exp",
    instruction=(
        "Fetch the top news story for the given topic and return only that story."
    ),
...
# The ParallelAgent will run the sub-agents in parallel.
...
# Execution Notes
# --------------------------------------------------------------------------
# When executed, both `weather_fetcher` and `news_fetcher` will run in parallel.
# Their outputs will be stored in the session state under "weather_data"
# and "news_data", respectively. The ParallelAgent manages synchronization.
...
# from google.genai import types  # Unused import removed; keep if you use Part/Blob later.

# -----------------------------------------------------------------------------
# 1) Simple function tool (separate actions from reasoning)
# -----------------------------------------------------------------------------
def generate_image(prompt: str) -> Dict[str, Any]:
    ...
    # In a real implementation, call your image generation API here.
    # For this example, we return mock image data.
    mock_image_bytes = b"mock_image_data_for_a_cat_wearing_a_hat"

    return {
        "status": "success",
        # The tool returns the raw bytes; the agent will handle Part creation.
...
# -----------------------------------------------------------------------------
# 2) ImageGeneratorAgent as an LlmAgent using the tool above
# -----------------------------------------------------------------------------
image_generator_agent = LlmAgent(
    ...
    instruction=(
        "You are an image generation specialist. Your task is to take the user's "
        "request and use the `generate_image` tool to create the image. The user's "
        "entire request should be used as the 'prompt' argument for the tool. "
...
# -----------------------------------------------------------------------------
# 3) Wrap the agent in an AgentTool (what the parent agent will call)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 4) Parent agent (orchestrator) that invents a prompt and calls ImageGen
# -----------------------------------------------------------------------------
artist_agent = LlmAgent(
    name="Artist",
    model="gemini-2.0-flash",
    instruction=(
        "You are a creative artist. First, invent a creative and descriptive "
        "prompt for an image. Then, use the `ImageGen` tool to generate the "
        "image using your prompt."
    ),
    tools=[image_tool],
)
...
<figure>
<img src="../imgs/fig.7.3.Multi-Agent_design_pattern.png" alt="Fig. 3: Multi-Agent design pattern">
<figcaption>
